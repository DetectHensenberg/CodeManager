...
(async () => {
 ...
 const data = await page.evaluate(() => [...document.querySelectorAll('#appsToolbar, #appsToolbar *')].map(el => { const r=el.getBoundingClientRect(); return {tag:el.tagName,id:el.id,cls:String(el.className),text:(el.innerText||el.getAttribute('placeholder')||'').trim().slice(0,60), x:r.x,y:r.y,w:r.width,h:r.height}; }));
