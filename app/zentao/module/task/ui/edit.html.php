...
if(!empty($task->team))
{
    $assignedToComponent = picker(setID('assignedTo'), setClass('w-full'), set::name('assignedTo'), set::value($task->assignedTo), set::items($assignedToOptions), set::required(true), $task->mode == 'linear' && !in_array($taskStatus, array('done', 'closed')) ? set::disabled(true) : null, $taskStatus == 'closed' ? set::disabled(true) : null);
}
else
{
