...
class docModel extends model
{
     ...
     * @return array
     */
    public function getAllSubSpaces($spaceType = 'all')
    {
     ...
     * @return array
     */
    public function getEditors($docID = 0, $objectType = 'doc')
    {
     ...
     * @return void
     */
    public static function forEachDocBlock($rawContent, $callback, $data = null, $flavours = '', $types = 'block', $props = null, $level = 0, $index = 0)
    {
        ...
        /* 判断内容是否匹配类型。 */
        $blockMatched = empty($types) || strpos(',' . $types . ',', ',' . $type . ',') !== false;

        /* 判断内容是否匹配 flavour。 */
        if($blockMatched && !empty($flavours))
        {
            $flavour      = empty($rawContent['flavour']) ? '' : $rawContent['flavour'];
            $blockMatched = !empty($flavour) && strpos(',' . $flavours . ',', ',' . $flavour . ',') !== false;
        }
