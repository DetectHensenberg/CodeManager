...
else
{
    ...
    (
        ...
        (
            ...
            cell(setClass('gap-2 w-2/5 flex-none items-center'), $progressCircle),
            cell
            (
...
), set::block($block), set::active($active), set::moreLink(hasPriv('execution', 'all') && $config->vision != 'lite' ? createLink('execution', 'all', 'status=' . zget($block->params, 'type', '')) : ''), set::items($items), $blockView);
