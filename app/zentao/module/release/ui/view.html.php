    ...
    sectionList(
        ...
        (
            ...
            (
                ...
                dtable(setID('finishedStoryDTable'), set::style(array('min-width' => '100%')), set::cols(array_values($config->release->dtable->story->fieldList)), set::data($storyTableData), set::userMap($users), set::checkable(empty($release->releases) && ($canBatchUnlinkStory || $canBatchCloseStory)), set::sortLink(createLink($releaseModule, 'view', "releaseID={$release->id}&type=story&link={$link}&param={$param}&orderBy={name}_{sortType}")), set::orderBy($orderBy), set::onRenderCell(jsRaw('window.renderStoryCell')), set::extraHeight('+144'), set::footToolbar($storyFootToolbar), set::footPager(usePager('storyPager', '', array('recPerPage' => $storyPager->recPerPage, 'recTotal' => $storyPager->recTotal, 'linkCreator' => createLink($releaseModule, 'view', "releaseID={$release->id}&type=story&link={$link}&param={$param}&orderBy={$orderBy}&recTotal={$storyPager->recTotal}&recPerPage={recPerPage}&page={page}")))), set::checkInfo(jsRaw("function(checkedIDList){return window.setStoryStatistics(this, checkedIDList, '{$summary}');}")))
            ),
