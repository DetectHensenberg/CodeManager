...
$generateData = function() use ($lang, $groupID, $pivotName, $pivot, $data, $configs, $showOrigin, $fnGenerateFilters, $hasVersionMark)
{
    ...
    (
        ...
        (
            ...
            (
                setClass(array('hidden' => $this->app->rawMethod == 'versions')),
                item(setID('origin-query'), setClass('ghost'), set::icon('exchange'), set::text($lang->pivot->showOrigin), on::click("toggleShowMode('origin')")),
                item(setID('pivot-query'), setClass('ghost hidden'), set::icon('exchange'), set::text($lang->pivot->showPivot), on::click("toggleShowMode('group')")),
                item(set(array
                (
...
JS
            ...
            div(setID('exportData'), setClass('hidden'), rawContent(), html($this->pivot->buildPivotTable($data, $configs)))
        )
    );
};
