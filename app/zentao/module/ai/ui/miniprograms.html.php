...
toolbar(common::hasPriv('ai', 'importMiniProgram') ? a(setClass('btn ghost'), setData('toggle', 'tooltip'), setData('title', $lang->ai->toZentaoStoreAIPage), setData('placement', 'left'), set::href($config->ai->storeUrl), set::target('_blank'), h::svg
(
    set::width('14'),
    set::height('14'),
...
), $lang->ai->store) : null, common::hasPriv('aiapp', 'square') ? item(set(array(
    'class' => 'secondary',
    'text'  => $lang->ai->exitManage,
    'url'   => createLink('aiapp', 'square'),
))) : null, common::hasPriv('ai', 'importMiniProgram') ? item(set(array(
    'class'       => 'primary',
    'icon'        => 'import',
    'text'        => $lang->ai->import,
    'data-toggle' => 'modal',
    'data-target' => '#importMiniProgramModal',
))) : null, $config->edition != 'open' && common::hasPriv('ai', 'createMiniProgram') ? item(set(array(
    'class' => 'primary',
    'icon'  => 'plus',
...
dtable(set::cols($cols), set::data($miniPrograms), set::orderBy($orderBy), set::sortLink(inlink('miniprograms', "category=$category&status=$status&orderBy={name}_{sortType}&recTotal={$pager->recTotal}&recPerPage={$pager->recPerPage}&pageID={$pager->pageID}")), set::footPager(usePager()), set::emptyTip($lang->ai->miniPrograms->emptyList));
