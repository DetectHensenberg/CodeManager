...
CSS;
    ...
    protected function buildSection($item, $title = null)
    {
        ...
        (
            ...
            div(setClass("detail-section-content py-1 {$className}"), $item ? new content(is_array($item) ? set($item) : $item) : null)
        );
    }
    ...
    protected function buildAITasks()
    {
        ...
        (
            ...
            dtable(setID('aitasks'), set::cols($cols), set::data($aiTasks), set::userMap($users), set::onRenderCell(jsRaw('window.renderAITaskCell')), set::emptyTip($lang->aitask->noTask))
        );
    }
