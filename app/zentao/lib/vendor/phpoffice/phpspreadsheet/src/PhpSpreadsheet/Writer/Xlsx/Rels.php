...
class Rels extends WriterPart
{
     ...
     */
    public function writeRelationships($spreadsheet)
    {
        // Create XML writer
        $objWriter = null;
     ...
     * @return string XML Output
     */
    public function writeWorkbookRelationships($spreadsheet)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
     ...
     * @return string XML Output
     */
    public function writeWorksheetRelationships($pWorksheet, $pWorksheetId = 1, $includeCharts = false)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
     ...
     */
    private function writeUnparsedRelationship($pWorksheet, $objWriter, $relationship, $type)
    {
        $unparsedLoadedData = $pWorksheet->getParent()->getUnparsedLoadedData();
     ...
     * @return string XML Output
     */
    public function writeDrawingRelationships($pWorksheet, &$chartRef, $includeCharts = false)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
     ...
     * @return string XML Output
     */
    public function writeHeaderFooterDrawingRelationships($pWorksheet)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
     ...
     * @throws WriterException
     */
    private function writeRelationship($objWriter, $pId, $pType, $pTarget, $pTargetMode = '')
    {
        if ($pType != '' && $pTarget != '') {
            // Write relationship
