...
class Xlsx extends BaseReader
{
     ...
     * @param \SimpleXMLElement $xmlWorkbook
     */
    private function readProtection($excel, $xmlWorkbook)
    {
     ...
     */
    private function readFormControlProperties($excel, $zip, $dir, $fileWorksheet, $docSheet, &$unparsedLoadedData)
    {
        if (!$zip->locateName(dirname("$dir/$fileWorksheet") . '/_rels/' . basename($fileWorksheet) . '.rels')) {
            return;
        }
     ...
     * @param mixed[] $unparsedLoadedData
     */
    private function readPrinterSettings($excel, $zip, $dir, $fileWorksheet, $docSheet, &$unparsedLoadedData)
    {
        if (!$zip->locateName(dirname("$dir/$fileWorksheet") . '/_rels/' . basename($fileWorksheet) . '.rels')) {
            return;
        }
