...
(
    ...
    (
        ...
        (
            ...
            inputGroup(span(setClass('input-group-addon' . (strpos(",{$config->ticket->create->requiredFields},", ',customer,') !== false ? ' required' : '')), $lang->ticket->customer), input
            (
                setClass('customerInput'),
                set::name('customer[0]'),
                set::value($customer)
            ), span(setClass('input-group-addon' . (strpos(",{$config->ticket->create->requiredFields},", ',contact,') !== false ? ' required' : '')), $lang->ticket->contact), input
            (
